...
Template.perfil.onCreated(function () {
  ...
  self.autorun(function () {
    let perfilId = FlowRouter.getParam('user');
    self.subscribe('muro', perfilId);
    self.subscribe('likes');
    self.subscribe('usuario');
...
Template.perfil.events({
      ...
      if ( texto !== "") {
        Meteor.call('postear', texto, function (err, result) {
            if (err) {
              ...
              template.find("[name='texto']").value = "";
              console.log('bien!');
            }
        });
      }
  ...
  'keyup .e': function (event, template) {
      ...
      if (event.which == 13) {
        ...
        // Maximo pueden ir 200 palabras en el estado -  crear script para
        // que ingrese maximo 200 palabras
        var estado = $('.e').text();
        console.log(estado);
  ...
  'click .agregar-amigo': function (event, template) {
    ...
    Meteor.call('nuevaSolicitud', datos, function (err) {
        if (err) {
          console.log(err);
        } else {
          console.log('listo');
  ...
  'keyup #comentar': function (event, template) {
      event.preventDefault();
...
Template.perfil.helpers({
  ...
  ytb: function () {
    ...
    if (match && match[2].length == 11) {
        var youtube = "https://www.youtube.com/embed/" + match[2] + "?autoplay=0";
        //$('#ytplayerSide').attr('src', 'https://www.youtube.com/embed/' + match[2] + '?autoplay=0');
        return youtube;
    }
  },
  esMiMuro: function () {
    if ( FlowRouter.getParam('user') === Meteor.userId() ) {
      return true;
    } else {
      return false;
    }
  ...
  likes: function () {
    ...
    return Likes.find({pubId: pubId}).fetch().length;
  ...
  corazon: function () {
    let like = Likes.find({pubId: this._id}).fetch();

    console.log(like);

    /*like.forEach(function (l) {
      if (l.de.id === Meteor.userId()) {
        console.log('yo');
