...
function subirFotoPrivado (event, template) {
          ...
          for (var i = 0; i < archivo.files.length; i++) {
            ...
            doc.metadata = {
              creadorId: Meteor.userId(),
						...
						let mensaje = "-";
						let para = {
				      id: FlowRouter.getParam('de')
				    };


						Meteor.call('enviarMensajePrivado', mensaje, para, function (error, result) {
							if (error) {
...
Template.listaDeUsuariosConectadosPrivados.onCreated(function () {
	var self = this;
...
Template.listaDeUsuariosConectadosPrivados.events({
  'click .user-card': function (event, template) {
    Meteor.call('Conversar', this._id, function (err) {
      if (err) {
        console.log(err);
      }
    });
...
Template.mensajesPrivados.onCreated( function () {
	var self = this;
...
Template.mensajesPrivados.onRendered(function () {
  setTimeout(function () {
     gotoBottom("mensajes4")
  }, 1000);
});

Template.mensajesPrivados.events({
  ...
  'change #myArchivo': function (event, template) {
    subirFotoPrivado(event, template);
    gotoBottom("mensajes4")
  }
});
...
Template.mensajesPrivados.helpers({
	mensajes: function () {
  ...
  username: function () {
    let userId = FlowRouter.getParam('de');
